...
LGFXMenuButton::LGFXMenuButton(LovyanGFX *displayPtr, uint16_t width, uint16_t height, uint16_t xOffset, uint16_t yOffset, uint8_t padding, uint8_t index, const char *topLabel, bool topLabelVisible, const char *bottomLabel, bool bottomLabelVisible, bool active, ButtonCallback onClick)
    : LGFXWidget(displayPtr, width, height, xOffset, yOffset, padding), index(index), topLabelVisible(topLabelVisible), bottomLabelVisible(bottomLabelVisible), active(active), onClick(onClick), changed(true)
{
...
#define NOT_ACTIVE_LABEL_COLOR 0x4A49
    ...
    if (changed)
    {
        ...
        uint16_t centeredTextXOffset = 0;
        if (this->topLabelVisible)
        {
            ...
            centeredTextXOffset = (this->width - (this->padding * 2) - this->parentDisplayPtr->textWidth(this->topLabel)) / 2;
        ...
        if (this->bottomLabelVisible)
        {
            ...
            uint16_t yOffSet = this->topLabelVisible ? _MENU_BUTTON_WIDGET_MAIN_LABEL_Y_OFFSET : _MENU_BUTTON_WIDGET_TOP_LABEL_Y_OFFSET;
            this->parentDisplayPtr->setCursor(this->xOffset + this->padding + centeredTextXOffset, this->yOffset + this->padding + yOffSet);
